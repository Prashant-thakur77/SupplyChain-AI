...
export default function Home() {
  ...
  return (
    <div className="min-h-screen overflow-x-hidden bg-[#F7F7F8] font-sans text-[#0B0B0F] antialiased">
      <header className={`sticky top-0 z-50 border-b transition-colors ${scrolled ? "border-[#E8E8EC] bg-[#F7F7F8]/85 backdrop-blur" : "border-transparent bg-transparent"}`}>
        <div className="mx-auto flex h-16 max-w-[1200px] items-center justify-between px-6">
          ...
          <nav className="hidden items-center gap-7 text-[14px] font-medium text-[#52525B] md:flex">
            {NAV.map(([h, l]) => <Link key={h} href={h} className="transition-colors hover:text-[#0B0B0F]">{l}</Link>)}
            <a href="https://github.com/Prashant-thakur77/SupplyChain-AI" target="_blank" rel="noreferrer" className="inline-flex items-center gap-1.5 transition-colors hover:text-[#0B0B0F]"><Github className="h-4 w-4" /> GitHub</a>
          </nav>
          <div className="flex items-center gap-2">
            <Link href="/signin" className="hidden rounded-full px-4 py-2 text-[14px] font-semibold text-[#0B0B0F] hover:bg-[#EFEFF2] md:inline-flex">Sign in</Link>
        ...
        {open && (
          <div className="border-t border-[#E8E8EC] bg-[#F7F7F8] px-6 py-4 md:hidden">
