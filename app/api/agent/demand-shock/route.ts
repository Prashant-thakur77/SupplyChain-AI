...
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    return NextResponse.json(await agentClient.post("/demand-shock", { supply_chain_id: twin.supply_chain_id, user_id: b.userId ?? (await getSessionUser())?.id ?? "anonymous", multiplier: Number(b.multiplier ?? 1.5), duration_weeks: Number(b.durationWeeks ?? 4), destination_ids: b.destinationIds ?? [] }))
