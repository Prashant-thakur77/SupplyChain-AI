    ...
    <Dialog open={isOpen} onOpenChange={(o) => !o && onClose()}>
      <DialogContent className="max-h-[90vh] overflow-y-auto sm:max-w-3xl">
        ...
        <div className="grid gap-3 rounded-theme-md border border-theme-border-subtle p-3 sm:grid-cols-[1fr_120px_1fr_auto] sm:items-end">
          <label className="text-xs text-theme-text-secondary">Demand change ({pct >= 0 ? "+" : ""}{pct}%)<input type="range" min={-50} max={200} step={5} value={pct} onChange={(e) => setPct(Number(e.target.value))} className="mt-2 w-full accent-[#2748E8]" /></label>
