...
export function Hero() {
  ...
  return (
    <section className="relative overflow-hidden bg-[#F6F3EE]">
      ...
      <div aria-hidden className="pointer-events-none absolute inset-0 bg-[linear-gradient(to_right,rgba(24,22,15,0.035)_1px,transparent_1px),linear-gradient(to_bottom,rgba(24,22,15,0.035)_1px,transparent_1px)] bg-[size:56px_56px] [mask-image:radial-gradient(ellipse_60%_60%_at_50%_30%,#000_30%,transparent_75%)]" />
      <div className="relative mx-auto grid max-w-[1200px] items-center gap-12 px-6 pb-20 pt-16 lg:grid-cols-[1.05fr_1fr] lg:gap-10 lg:pb-28 lg:pt-24">
        <div>
          ...
          <h1 className="mt-6 font-display text-[clamp(2.3rem,4.6vw,3.9rem)] font-semibold leading-[1.04] tracking-[-0.025em] text-[#18160F]">
            {words.map((w, i) => (
          ...
          <motion.p initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.55, ease }} className="mt-6 max-w-[52ch] text-[1.08rem] leading-relaxed text-[#5C5850]">
            An autonomous agent watches your ports, suppliers and lanes. When something breaks it computes the blast radius, finds the cheapest viable reroute, drafts the mitigation — and pings you with <strong className="font-semibold text-[#18160F]">one ranked decision</strong> to approve.
          </motion.p>
          <motion.div initial={reduce ? false : { opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.6, delay: 0.7, ease }} className="mt-8 flex flex-wrap items-center gap-3">
            ...
            <Link href="/signin" className="inline-flex items-center gap-2 rounded-full border border-[#18160F]/15 bg-white/60 px-6 py-3 text-[15px] font-semibold text-[#18160F] backdrop-blur transition-colors hover:border-[#18160F]/40">Sign in</Link>
          </motion.div>
          <motion.div initial={reduce ? false : { opacity: 0 }} animate={{ opacity: 1 }} transition={{ duration: 0.6, delay: 0.9 }} className="mt-8 flex flex-wrap items-center gap-x-5 gap-y-2 text-[12px] text-[#5C5850]">
            <span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#1A7F4B]" /> No login for the demo</span>
            <span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#1A7F4B]" /> Open source · MIT</span>
            <span className="inline-flex items-center gap-1.5"><Check className="h-3.5 w-3.5 text-[#1A7F4B]" /> Gemini · Bedrock · Groq · local Ollama</span>
          </motion.div>
        </div>
        <motion.div initial={reduce ? false : { opacity: 0, scale: 0.97, y: 16 }} animate={{ opacity: 1, scale: 1, y: 0 }} transition={{ duration: 0.8, delay: 0.35, ease }}
          className="rounded-[28px] border border-[#E5DFD6] bg-white/70 p-4 shadow-[0_30px_80px_-30px_rgba(24,22,15,0.35)] backdrop-blur sm:p-6">
...
export function Ticker() {
  return (
    <section className="border-y border-[#E5DFD6] bg-[#EFEBE3]">
      <div className="mx-auto max-w-[1200px] px-6 py-3">
        <div className="mb-1 text-center text-[10px] font-bold uppercase tracking-[0.18em] text-[#5C5850]">What Sentinel reads so you don't have to · every 15 minutes</div>
        <Marquee speed={40} className="text-[13px] text-[#18160F]">
          {TICKER.map(([k, t], i) => (
...
export function Problem() {
  return (
    <section className="bg-[#F6F3EE]" id="problem">
      <div className="mx-auto grid max-w-[1200px] gap-12 px-6 py-20 lg:grid-cols-2 lg:gap-16 lg:py-28">
        <Reveal>
          ...
          <h2 className="mt-3 font-display text-[clamp(1.9rem,3.4vw,2.8rem)] font-semibold leading-[1.08] tracking-[-0.02em] text-[#18160F]">You find out from the news. Then you lose a day.</h2>
          <p className="mt-4 max-w-[48ch] text-[1.02rem] leading-relaxed text-[#5C5850]">Port closures, strikes and typhoons don't wait for office hours. Working out what they hit, what the alternatives cost and who to call is repetitive <em>and</em> judgement-heavy — the worst kind of work to start from zero at 2 a.m.</p>
          <ul className="mt-6 space-y-3">
            {["No real-time view of which lanes are actually exposed", "Reroute costs guessed in spreadsheets, never audited", "Every incident handled from scratch — nothing learned"].map((t) => (
              <li key={t} className="flex items-start gap-3 text-[15px] text-[#18160F]"><span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#FEE2E2] text-[#B91C1C]"><X className="h-3 w-3" /></span>{t}</li>
            ))}
          </ul>
        </Reveal>
        <Reveal delay={0.1}>
          <div className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#2748E8]">What the agent does instead</div>
          <h2 className="mt-3 font-display text-[clamp(1.9rem,3.4vw,2.8rem)] font-semibold leading-[1.08] tracking-[-0.02em] text-[#18160F]">Background by default. One card when it matters.</h2>
          <ul className="mt-6 space-y-3">
            {["Sentinel scans news and weather for every node and lane, on a schedule", "Analyst grades severity and blast radius — below HIGH, nobody is interrupted", "A deterministic routing engine computes the real alternatives; the model only ranks and explains", "Approve, reject or snooze in the Decision Inbox — it records, notifies and remembers"].map((t) => (
              <li key={t} className="flex items-start gap-3 text-[15px] text-[#18160F]"><span className="mt-0.5 flex h-5 w-5 shrink-0 items-center justify-center rounded-full bg-[#DCFCE7] text-[#1A7F4B]"><Check className="h-3 w-3" /></span>{t}</li>
            ))}
          </ul>
          <Link href="/demo" className="mt-8 inline-flex items-center gap-2 rounded-full bg-[#18160F] px-5 py-2.5 text-[14px] font-semibold text-white transition-transform hover:-translate-y-0.5">See it break a port <ArrowRight className="h-4 w-4" /></Link>
        </Reveal>
      </div>
    </section>
  )
...
export function HowItWorks() {
  return (
    <section className="bg-[#F6F3EE]" id="how-it-works">
      <div className="mx-auto max-w-[1200px] px-6 py-20 lg:py-28">
        <Reveal className="max-w-[60ch]">
          <div className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#2748E8]">How it works</div>
          <h2 className="mt-3 font-display text-[clamp(1.9rem,3.4vw,2.8rem)] font-semibold leading-[1.08] tracking-[-0.02em] text-[#18160F]">Four steps. Three of them happen while you sleep.</h2>
        </Reveal>
        <RevealGroup className="mt-12 grid gap-4 md:grid-cols-2 lg:grid-cols-4">
          {STEPS.map(({ icon: Icon, t, d }, i) => (
            <RevealItem key={t} className="group relative rounded-2xl border border-[#E5DFD6] bg-white p-6 transition-all hover:-translate-y-1 hover:shadow-[0_20px_50px_-24px_rgba(24,22,15,0.35)]">
              <div className="flex items-center justify-between"><span className="flex h-10 w-10 items-center justify-center rounded-xl bg-[#EEF1FF] text-[#2748E8]"><Icon className="h-5 w-5" /></span><span className="font-display text-2xl text-[#E5DFD6] transition-colors group-hover:text-[#2748E8]/30">0{i + 1}</span></div>
              <h3 className="mt-5 font-display text-xl font-semibold text-[#18160F]">{t}</h3>
              <p className="mt-2 text-[14px] leading-relaxed text-[#5C5850]">{d}</p>
            </RevealItem>
          ))}
        </RevealGroup>
      </div>
    ...
    <section className="border-y border-[#E5DFD6] bg-[#EFEBE3]">
      <div className="mx-auto grid max-w-[1200px] items-center gap-12 px-6 py-20 lg:grid-cols-[1fr_1.1fr] lg:py-28">
        ...
        <RevealGroup className="grid grid-cols-2 gap-3 sm:grid-cols-4 lg:grid-cols-2 xl:grid-cols-4">
          {AGENTS.map(([n, r, c]) => (
            <RevealItem key={n} className="rounded-2xl border border-[#E5DFD6] bg-white p-4 transition-transform hover:-translate-y-1">
              <span className="block h-2 w-2 rounded-full" style={{ background: c }} />
              <div className="mt-3 font-display text-[17px] font-semibold text-[#18160F]">{n}</div>
              <div className="mt-1 text-[12px] leading-snug text-[#5C5850]">{r}</div>
            </RevealItem>
          ))}
        </RevealGroup>
      </div>
...
export function InboxPreview() {
  return (
    <section className="bg-[#F6F3EE]" id="inbox">
      <div className="mx-auto grid max-w-[1200px] items-center gap-12 px-6 py-20 lg:grid-cols-[1.1fr_1fr] lg:py-28">
        <Reveal className="order-2 lg:order-1">
          <div className="rounded-[24px] border border-[#E5DFD6] bg-white p-5 shadow-[0_30px_80px_-40px_rgba(24,22,15,0.4)]">
            <div className="flex items-center gap-2 text-[11px] font-bold uppercase tracking-[0.14em] text-[#5C5850]"><Inbox className="h-3.5 w-3.5" /> Decision Inbox · 1 pending</div>
            <div className="mt-4 rounded-2xl border-l-4 border-[#B45309] bg-[#F6F3EE] p-4">
              <div className="flex flex-wrap items-center gap-2 text-[10px] font-bold uppercase tracking-wide"><span className="rounded-full bg-[#FEF3C7] px-2 py-0.5 text-[#B45309]">High</span><span className="text-[#5C5850]">2 minutes ago</span><span className="ml-auto rounded-full border border-[#1A7F4B]/30 bg-[#DCFCE7] px-2 py-0.5 text-[#1A7F4B]">95% · 2 sources</span></div>
              <div className="mt-2 font-display text-[19px] font-semibold leading-tight text-[#18160F]">Port of Singapore closed for 3 weeks: choose a route</div>
              {[["Shenzhen → Colombo → Suez → Rotterdam → Berlin", "+$1,000 · +5d · low risk", true], ["Shenzhen → Colombo → Suez → Hamburg → Berlin", "+$1,000 · +6d · low risk", false], ["Wait and monitor", "+21d · high risk", false]].map(([l, s, rec]) => (
                <div key={l as string} className={`mt-2 flex items-center justify-between gap-3 rounded-xl border px-3 py-2 ${rec ? "border-[#2748E8] bg-[#EEF1FF]" : "border-[#E5DFD6] bg-white"}`}>
                  <span className="truncate text-[13px] font-medium text-[#18160F]">{l}</span><span className="shrink-0 text-[11px] text-[#5C5850]">{s}</span>
                </div>
              ))}
              <div className="mt-3 flex gap-2"><span className="inline-flex items-center gap-1.5 rounded-full bg-[#2748E8] px-4 py-1.5 text-[12px] font-semibold text-white"><Check className="h-3.5 w-3.5" /> Approve</span><span className="rounded-full border border-[#E5DFD6] px-4 py-1.5 text-[12px] font-semibold text-[#5C5850]">Reject</span><span className="rounded-full px-3 py-1.5 text-[12px] font-semibold text-[#5C5850]">Snooze 24h</span></div>
            </div>
            <div className="mt-3 flex items-center gap-2 rounded-xl border border-[#2748E8]/20 bg-[#EEF1FF]/60 px-3 py-2 text-[12px] text-[#5C5850]"><Sparkles className="h-3.5 w-3.5 text-[#2748E8]" /> Last time this happened: rerouted via Colombo, +$1,000, delivered 4 days early.</div>
          </div>
        </Reveal>
        <Reveal delay={0.1} className="order-1 lg:order-2">
          <div className="text-[11px] font-bold uppercase tracking-[0.16em] text-[#2748E8]">Human in the loop</div>
          <h2 className="mt-3 font-display text-[clamp(1.9rem,3.4vw,2.8rem)] font-semibold leading-[1.08] tracking-[-0.02em] text-[#18160F]">Not another dashboard to babysit. An inbox that's usually empty.</h2>
