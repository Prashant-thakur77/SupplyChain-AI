    ...
    <div className={`relative ${className}`} aria-label="Animated demo: a port fails, the agent reroutes and asks for one decision">
      <svg viewBox="0 0 700 300" className="h-auto w-full" role="img">
        <defs>
          ...
          <pattern id="grid" width="28" height="28" patternUnits="userSpaceOnUse"><path d="M 28 0 L 0 0 0 28" fill="none" stroke="#18160F" strokeOpacity="0.05" strokeWidth="1" /></pattern>
        </defs>
        ...
        {LANES.map((l) => {
          ...
          return (
            <g key={key(l)}>
              <path d={path(l)} fill="none" stroke="#18160F" strokeOpacity={dead ? 0.12 : 0.18} strokeWidth={2} />
              ...
              {active && (
                <motion.path d={path(l)} fill="none" stroke="#1A7F4B" strokeWidth={3.5} strokeLinecap="round" filter="url(#glow)"
        ...
        {(Object.keys(N) as Id[]).map((id) => {
          ...
          const stroke = isFailed ? "#B91C1C" : onRoute ? "#1A7F4B" : downstream ? "#B45309" : "#18160F"
          return (
            <g key={id} transform={`translate(${n.x}, ${n.y})`}>
              ...
              <text y={30} textAnchor="middle" fontSize="12" fontWeight={600} fill="#18160F" fontFamily="Inter, system-ui, sans-serif">{n.label}</text>
              <text y={43} textAnchor="middle" fontSize="9.5" fill="#5C5850" fontFamily="Inter, system-ui, sans-serif" letterSpacing="0.06em">{n.sub.toUpperCase()}</text>
            </g>
          )
        })}
      </svg>
      ...
      {/* agent status pill */}
      <div className="absolute left-2 top-2 flex items-center gap-2 rounded-full border border-[#E5DFD6] bg-white/90 px-3 py-1 text-[11px] font-semibold text-[#5C5850] shadow-sm backdrop-blur">
        <span className={`h-1.5 w-1.5 rounded-full ${failed ? "bg-[#B91C1C]" : "bg-[#1A7F4B]"} ${failed && !reduce ? "animate-pulse" : ""}`} />
        ...
        <AnimatePresence mode="wait">
          {showCard ? (
            <motion.div key="card" initial={{ opacity: 0, y: 14 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, y: 6 }} transition={{ duration: 0.45, ease: [0.16, 1, 0.3, 1] }}
              className="flex flex-col gap-3 rounded-2xl border border-[#E5DFD6] bg-white p-3 shadow-[0_18px_40px_-24px_rgba(24,22,15,0.35)] sm:flex-row sm:items-center">
              <div className="min-w-0 flex-1">
                ...
                <div className="mt-1.5 rounded-xl border border-[#2748E8] bg-[#EEF1FF] px-3 py-1.5">
                  <div className="truncate text-[12.5px] font-semibold text-[#18160F]">Shenzhen → Colombo → Suez → Rotterdam → Berlin</div>
                  <div className="mt-0.5 flex items-center gap-1.5 text-[11px] text-[#5C5850]"><span className="h-1.5 w-1.5 rounded-full bg-[#1A7F4B]" />+$1,000 · +5d · low risk · <span className="font-semibold text-[#1A7F4B]">recommended</span> · vs wait +21d</div>
                </div>
              </div>
              <motion.div className="flex shrink-0 items-center justify-center gap-1.5 rounded-full px-4 py-2.5 text-[12px] font-semibold text-white sm:w-[190px]" animate={{ backgroundColor: phase === "approved" ? "#1A7F4B" : "#2748E8" }}>
                <Check className="h-3.5 w-3.5" /> {phase === "approved" ? "Approved · remembered" : "Approve reroute"}
              </motion.div>
            </motion.div>
          ) : (
            <motion.div key="idle" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="flex h-[92px] items-center justify-center rounded-2xl border border-dashed border-[#E5DFD6] text-[12px] text-[#5C5850]">
