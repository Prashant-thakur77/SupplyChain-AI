...
const LABELS: Record<string, { title: string; hint: string }> = {
  ...
  contracts: { title: "Contracts & SLAs", hint: "Penalty clauses on affected sites — deterministic, no LLM" },
    ...
    <div className={cn("rounded-theme-lg border border-theme-border-subtle bg-theme-bg-surface", className)}>
      ...
      <ol className={cn("space-y-1 p-2", compact ? "max-h-56 overflow-y-auto" : "max-h-[60vh] overflow-y-auto")}>
        ...
        {steps.map((s, i) => {
          ...
          const Icon = s.node === "routing_engine" || s.node === "inventory" || s.node === "contracts" ? Route : s.node.startsWith("graph") ? Cpu : Bot
          const isOpen = !!open[i]
          return (
